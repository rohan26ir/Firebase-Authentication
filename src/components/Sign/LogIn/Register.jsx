...
const Register = () => {
  ...
  const handleCreate = (event) => {
    ...
    if(!passwordRegex.test(password)){
      setErrorMassage('At least one uppercase, one lowercase, one number, one special character');
      return;
    }
    ...
    <div>
      ...
      <div className="w-[30%] mx-auto my-10">
        <div className="card bg-white w-full max-w-sm shrink-0 shadow-2xl">
          ...
          {
            errorMassage && <p className="text-red-600 text-center mb-2">{errorMassage}</p>
          }
          {/* Successful message */}

          {/* if  account is create then show */}
          {
            success && <p className="text-green-500 text-center mb-2">Sign up is Successful.</p>
          }
